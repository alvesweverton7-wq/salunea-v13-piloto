-- B3C — recorrência por visita comercial
-- Correção incremental: múltiplos atendimentos concluídos do mesmo cliente no mesmo dia
-- contam como UMA visita para o cálculo de recorrência.
...
begin
  ...
  if not exists (
    ...
    where c.id = p_client_id and c.company_id = p_company_id and c.archived_at is null
  ) then raise exception 'Cliente não encontrado nesta empresa.'; end if;

  -- Uma visita comercial por data-calendário UTC do atendimento concluído.
  -- Mantemos o último completed_at do dia como timestamp representativo.
  with commercial_days as (
    select (a.completed_at at time zone 'UTC')::date as visit_day,
    ...
    group by (a.completed_at at time zone 'UTC')::date
  ), sequenced as (
    select visit_day, completed_at,
           lag(visit_day) over (order by visit_day) as previous_day
    from commercial_days
  )
  select count(*), min(completed_at), max(completed_at),
         round(avg((visit_day - previous_day)::numeric), 2)
    into v_visits, v_first, v_last, v_avg_days
  from sequenced;

  -- Financeiro continua somando comandas reais; consolidar visita não apaga serviços/receita.
  ...
  insert into public.client_recurrence_profiles(
    company_id, client_id, completed_visits, first_completed_at, last_completed_at,
    average_return_days, next_expected_at, days_since_last_visit, average_ticket,
    lifetime_revenue, recurrence_status, calculated_at
  ) values (
    p_company_id,p_client_id,v_visits,v_first,v_last,v_avg_days,v_next,v_days,
    v_avg_ticket,v_lifetime,v_status,now()
