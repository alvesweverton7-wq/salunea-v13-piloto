-- B3E — métricas econômicas da recorrência
-- Receita histórica = pagamentos alocados às comandas do cliente - estornos dessas comandas.
-- Ticket médio = receita líquida / quantidade de visitas comerciais concluídas.
...
declare
  ...
  v_visits integer:=0; v_first timestamptz; v_last timestamptz;
...
begin
  ...
  with commercial_days as (
    select (a.completed_at at time zone coalesce(u.timezone,'America/Sao_Paulo'))::date visit_day,
           max(a.completed_at) completed_at
    from public.attendances a
    left join public.units u on u.id=a.unit_id and u.company_id=a.company_id
    where a.company_id=p_company_id and a.client_id=p_client_id
      and a.status='completed' and a.completed_at is not null
    group by (a.completed_at at time zone coalesce(u.timezone,'America/Sao_Paulo'))::date
  ), sequenced as (
  ...
  -- Agrega cada lado separadamente para impedir multiplicação de linhas entre allocations e refunds.
  with client_orders as (
    ...
    select coalesce(sum(pa.amount),0)::numeric total
    from public.payment_allocations pa join client_orders co on co.id=pa.order_id
  ), refunded as (
    select coalesce(sum(pr.amount),0)::numeric total
    from public.payment_refunds pr join client_orders co on co.id=pr.order_id
  )
  select greatest(0,paid.total-refunded.total) into v_lifetime from paid,refunded;
