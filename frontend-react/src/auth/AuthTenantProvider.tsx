...
export function AuthTenantProvider({children}:{children:ReactNode}){
 ...
 async function signIn(email:string,password:string){const {error}=await supabase.auth.signInWithPassword({email,password});if(error)return error.message;await bootstrap();return null;}
 ...
 useEffect(()=>{void bootstrap();const {data:{subscription}}=supabase.auth.onAuthStateChange((_event,session)=>{if(!session?.user)setState({...initial,status:'signed_out'});});return()=>subscription.unsubscribe();},[]);
