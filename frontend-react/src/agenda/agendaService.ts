...
export type AgendaItem={id:string;client_id:string|null;starts_at:string;ends_at:string;status:string;notes:string|null};
...
export async function listDayAppointments(companyId:string,unitId:string,startIso:string,endIso:string){const {data,error}=await supabase.from('appointments').select('id,client_id,starts_at,ends_at,status,notes').eq('company_id',companyId).eq('unit_id',unitId).gte('starts_at',startIso).lt('starts_at',endIso).order('starts_at');if(error)throw error;return (data??[]) as AgendaItem[];}
