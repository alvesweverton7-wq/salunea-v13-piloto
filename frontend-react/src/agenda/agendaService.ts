...
export async function listAgendaLabels(companyId:string,items:AgendaItem[]):Promise<AgendaLabels>{const serviceIds=[...new Set(items.map(x=>x.service_id).filter((x):x is string=>Boolean(x)))],professionalIds=[...new Set(items.map(x=>x.professional_id).filter((x):x is string=>Boolean(x)))];const [serviceResult,professionalResult]=await Promise.all([serviceIds.length?supabase.from('services').select('id,name').eq('company_id',companyId).in('id',serviceIds):Promise.resolve({data:[],error:null}),professionalIds.length?supabase.from('professionals').select('id,display_name,full_name').eq('company_id',companyId).in('id',professionalIds):Promise.resolve({data:[],error:null})]);if(serviceResult.error||professionalResult.error)throw serviceResult.error||professionalResult.error;return{services:Object.fromEntries((serviceResult.data??[]).map((x:any)=>[x.id,x.name])),professionals:Object.fromEntries((professionalResult.data??[]).map((x:any)=>[x.id,x.display_name||x.full_name||'Profissional']))};}
